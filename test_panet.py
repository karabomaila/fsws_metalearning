...
def fast_tune(sup_img, sup_msk, qry_loader, model, loss_name, device):
    # Time counters.
    adapt_time = 0.0
    test_time = 0.0

    # Setting model to evaluation mode (no tuning required).
    model.eval()
    with torch.no_grad():
        # Adapt the model.
...
def main(
    ...
    exp_group_name = "panet_%s_%s_%s_%s" % (
        network_name,
        loss_name,
        trg_dataset,
        trg_task,
    ...
    log_path = os.path.join(exp_group_dir, "test_panet.log")
