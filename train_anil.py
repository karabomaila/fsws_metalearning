...
def fast_adapt(
    sup_img, sup_msk, qry_img, qry_msk, model, head, adaptation_steps, loss_name, device
):
    # Adapt the model.
...
def main(
    ...
    log_path = os.path.join(exp_group_dir, "train_anil.log")
    ...
    while iteration <= outer_iters:
        for outer_batch in meta_loader:
            ...
            for task in range(inner_iters):
                ...
                learner = head.clone()
                sup_err, qry_err, qry_met = fast_adapt(
                    inner_sup_img,
                    inner_sup_msk,
                    inner_qry_img,
                    inner_qry_msk,
