...
def fast_tune(
    sup_img, sup_msk, qry_loader, model, model_msk, pool, head, loss_name, device
):
    # Time counters.
    adapt_time = 0.0
    test_time = 0.0
    ...
    with torch.no_grad():
        ...
        sup_emb = torch.tile(sup_cat_emb, (1, 1, sup_img.shape[2], sup_img.shape[3]))
...
def meta_test(
    model, model_msk, pool, head, loss_name, device, exp_group_dir, exp, sparsity_mode
):
...
def main(
    ...
    exp_group_name = "guided_net_2ch_%s_%s_%s_%s" % (
        network_name,
        loss_name,
        trg_dataset,
        trg_task,
    ...
    for fold in folds:
        ...
        if "unet" in network_name:
            ...
            # Create classifier.
            head = nn.Sequential(
                nn.Conv2d(unet_base * 16 * 2, unet_base * 2, kernel_size=3, padding=1),
                nn.ReLU(),
            ...
            head = nn.Sequential(
                nn.Conv2d(
                    model.final_input_channels * 2,
                    model.final_input_channels,
                    kernel_size=3,
                    padding=1,
                ),
                nn.ReLU(),
