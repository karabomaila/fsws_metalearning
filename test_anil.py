...
def fast_tune(
    sup_img, sup_msk, qry_loader, model, head, adaptation_steps, loss_name, device
):
    # Time counters.
    adapt_time = 0.0
    test_time = 0.0
    ...
    features_sup = model(sup_img)

    for step in range(adaptation_steps):
        sup_prd = head(features_sup)
    ...
    with torch.no_grad():
        ...
        for batch in qry_loader:
            ...
            features_qry = model(qry_img)

            qry_prd = head(features_qry)
...
def meta_test(
    model, head, adaptation_steps, loss_name, device, exp_group_dir, exp, sparsity_mode
):
...
def main(
    ...
    adaptation_steps=10,  # Number of adaptation steps at each inner loop.
    ...
    exp_group_name = "anil_%s_%s_%s_%s" % (
        network_name,
        loss_name,
        trg_dataset,
        trg_task,
    ...
    log_path = os.path.join(exp_group_dir, "test_anil.log")
    ...
    for fold in folds:
        ...
        if "unet" in network_name:
            ...
            # Segmentation head.
            head = [nn.Conv2d(unet_base, 2, kernel_size=1)]

            # Enveloping into MAML wrapper.
            head = nn.Sequential(*head)
            head = l2l.algorithms.MAML(head, lr=fast_lr, first_order=False)
            head.to(device)
            ...
            # Segmentation head.
            head = [nn.Conv2d(model.final_input_channels, 2, kernel_size=1)]
