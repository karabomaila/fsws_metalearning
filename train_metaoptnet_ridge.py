...
def fast_adapt(
    sup_img, sup_msk, qry_img, qry_msk, model, head, eps_smooth, loss_name, device
):
    ...
    smoothed_one_hot = smoothed_one_hot * (1 - eps_smooth) + (
        1 - smoothed_one_hot
    ) * eps_smooth / (2 - 1)
...
def main(
    ...
    if len(sys.argv) >= 6:
        pretrained = sys.argv[
            5
        ]  # Optional parameter with the path to pretrained with SSL renset18 or resnet50 model
    ...
    log_path = os.path.join(exp_group_dir, "train_metaoptnet_ridge.log")
    ...
    while iteration <= outer_iters:
        for outer_batch in meta_loader:
            ...
            for task in range(inner_iters):
                print(
                    '  Inner loop %d/%d, task "%s", mode "%s"...'
                    % (
                        task + 1,
                        inner_iters,
                        sparsity_list[task]["task"],
                        sparsity_list[task]["mode"],
                    )
                )
                logging.info(
                    '  Inner loop %d/%d, task "%s", mode "%s"...'
                ...
                if (
                    torch.any(inner_sup_msk == 0)
                    and torch.any(inner_sup_msk == 1)
                    and torch.any(inner_qry_msk == 0)
                    and torch.any(inner_qry_msk == 1)
                ):
                    # Zeroing optimizer gradients.
                    opt.zero_grad()

                    # Compute meta-training loss.
                    qry_err, qry_met = fast_adapt(
                        inner_sup_img,
                        inner_sup_msk,
                        inner_qry_img,
                        inner_qry_msk,
