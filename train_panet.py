...
def fast_adapt(sup_img, sup_msk, qry_img, qry_msk, model, loss_name, device):
    # Adapt the model.
    ...
    qry_err = loss_fn(logits_qry, lin_qry_msk, weights, loss_name, device) + loss_fn(
        logits_sup, lin_sup_msk, weights, loss_name, device
    )
...
def main(
    ...
    if len(sys.argv) >= 6:
        pretrained = sys.argv[
            5
        ]  # Optional parameter with the path to a renset18 or resnet50 model pretrained with SSL
