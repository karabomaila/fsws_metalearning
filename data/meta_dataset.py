...
from skimage import exposure, io, measure, transform
...
class MetaDataset(data.Dataset):
    ...
    def __init__(
        ...
        for i, d in enumerate(self.dataset_list):
            if self.verbose:
                print(
                    'Configuring dataset %d/%d: "%s"'
                    % (i + 1, len(self.dataset_list), d)
                )
                logging.info(
                    'Configuring dataset %d/%d: "%s"'
                    % (i + 1, len(self.dataset_list), d)
    ...
    def make_dataset(self, dataset):
        ...
        for t in tasks:
            ...
            sup_data_list = [
                ...
                for l in open(
                    os.path.join(
                        self.root, dataset, "folds", "trn_%s_f%d.txt" % (t, self.fold)
                    )
                ).readlines()
            ]
            qry_data_list = [
                ...
                for l in open(
                    os.path.join(
                        self.root, dataset, "folds", "tst_%s_f%d.txt" % (t, self.fold)
                    )
                ).readlines()
            ]
    ...
    def slicing_trn(self, img, msk, augmentation, torch_random=True):
        ...
        img = transform.resize(
            img, self.resize_to, order=1, preserve_range=True, anti_aliasing=False
        )
        msk = transform.resize(
            msk, self.resize_to, order=0, preserve_range=True, anti_aliasing=False
        ).astype(np.int64)

        # Cropping to size self.crop_to.
        img = img[
            augmentation["offset_0"] : (augmentation["offset_0"] + self.crop_to[0]),
    ...
    ################################################################
    def read_sample(
        self, img_path, msk_path, augmentation, sparsity_mode, sparsity_param, seed
    ):
        # Reading images.
        ...
        if sparsity_mode == "points":
            sparse_msk, bundle = sparse_points(
                msk,
            ...
            sparse_msk, bundle = sparse_scribbles(
                msk,
                sparsity_param=sparsity_param,
                torch_random=True,
                seed=seed,
            ...
            sparse_msk, bundle = sparse_contours(
                msk,
                sparsity_param=sparsity_param,
                torch_random=True,
                seed=seed,
            ...
            sparse_msk, bundle = sparse_skeleton(
                msk,
                sparsity_param=sparsity_param,
                torch_random=True,
                seed=seed,
            ...
            sparse_msk, bundle = sparse_polygon(
                msk,
                sparsity_param=sparsity_param,
                torch_random=True,
                seed=seed,
    ...
    def apply_zoom(self, img, msk, augmentation):
        ...
        if self.verbose:
            print(
                "Old ROI: (%d..%d, %d..%d)"
                % (roi_2d[0], roi_2d[2], roi_2d[1], roi_2d[3])
            )
            logging.info(
                "Old ROI: (%d..%d, %d..%d)"
                % (roi_2d[0], roi_2d[2], roi_2d[1], roi_2d[3])
        ...
        margins_2d = (
            int(round(roi_2d[0] * (1.0 - augmentation["zoom"]))),
        ...
        if self.verbose:
            print(
                "New ROI: (%d..%d, %d..%d)"
                % (new_roi_2d[0], new_roi_2d[2], new_roi_2d[1], new_roi_2d[3])
            )
            logging.info(
                "New ROI: (%d..%d, %d..%d)"
                % (new_roi_2d[0], new_roi_2d[2], new_roi_2d[1], new_roi_2d[3])
        ...
        new_img = img[new_roi_2d[0] : new_roi_2d[2], new_roi_2d[1] : new_roi_2d[3]]
        new_msk = msk[new_roi_2d[0] : new_roi_2d[2], new_roi_2d[1] : new_roi_2d[3]]
    ...
    def random_augmentation(self, torch_random=True):
        ...
        if torch_random:
            augmentation = {
                "zoom": torch.rand(1).item() * 0.2,
                "offset_0": torch.randint(
                    0, self.resize_to[0] - self.crop_to[0], (1,)
                ).item(),
                "offset_1": torch.randint(
                    0, self.resize_to[1] - self.crop_to[1], (1,)
                ).item(),
                "flip_0": torch.rand(1).item() > 0.5,
                "flip_1": torch.rand(1).item() > 0.5,
                "rot_90": torch.randint(0, 4, (1,)).item(),
                "rot_angle": torch.randn(1).item() * 4
    ...
    def __getitem__(self, index):
        ...
        curr_bundle = {
            "task": tsk_dict["dataset"] + " " + tsk_dict["task"],
            "mode": sparsity_mode,
            "sup": [],
        }
        ...
        for sup_i, ps in enumerate(perm_sup):
            ...
            # Reading samples.
            img, msk, sparse_msk, bundle = self.read_sample(
                img_path, msk_path, augmentation, sparsity_mode, sparsity_param, seed
            )
        ...
        for qry_i, pq in enumerate(perm_qry):
            ...
            # Reading samples.
            img, msk, sparse_msk, bundle = self.read_sample(
                img_path, msk_path, augmentation, "dense", None, seed
            )
