...
def fast_tune(sup_img, sup_msk, qry_loader, model, adaptation_steps, loss_name, device):
    # Time counters.
    adapt_time = 0.0
    test_time = 0.0
...
def meta_test(
    model, adaptation_steps, loss_name, device, exp_group_dir, exp, sparsity_mode
):
...
def main(
    ...
    exp_group_name = "maml_%s_%s_%s_%s" % (
        network_name,
        loss_name,
        trg_dataset,
        trg_task,
    ...
    log_path = os.path.join(exp_group_dir, "test_maml.log")
    ...
    for fold in folds:
        ...
        meta_test(
            model, adaptation_steps, loss_name, device, exp_group_dir, exp_grid, "grid"
        )
        meta_test(
            model,
            adaptation_steps,
        ...
        meta_test(
            model, adaptation_steps, loss_name, device, exp_group_dir, exp_dens, "dense"
        )
