...
def fast_tune(sup_img, sup_msk, qry_loader, model, head, loss_name, device):
    # Time counters.
    adapt_time = 0.0
    test_time = 0.0

    # Setting model and head to evaluation mode (no tuning required on MetaOptNet-Ridge).
    model.eval()
    head.eval()
    with torch.no_grad():
        # Tic.
        ...
        sup_emb = torch.reshape(
            sup_emb, (-1, sup_emb.shape[3])
        )  # [B, H, W, C] -> [B * H * W, C]
        ...
        lin_sup_msk = (
            torch.zeros((sup_emb.shape[1]), dtype=torch.long).unsqueeze(0).to(device)
        )
        ...
        if torch.any(lin_sup_msk == 0) and torch.any(lin_sup_msk == 1):
            ...
            for batch in qry_loader:
                ...
                qry_emb = torch.reshape(
                    qry_emb, (-1, qry_emb.shape[3])
                )  # [B, H, W, C] -> [B * H * W, C]

                # Concating negative and positive prototypes on support.
                qry_emb = qry_emb.unsqueeze(0)

                # Training SVM on support and predicting query logits.
                logits_qry = head(
                    qry_emb,
                    sup_emb,
                    lin_sup_msk,
                ...
                if torch.any(torch.isnan(qry_err)):
                    ...
                    qry_prd = torch.zeros(
                        (2, qry_msk.shape[-2], qry_msk.shape[-1]), dtype=torch.float32
                    )
                    qry_prd[0, :, :] = 1.0
                    ...
                    qry_prd = logits_qry.view(
                        qry_msk.shape[-2], qry_msk.shape[-1], 2
                    ).permute(2, 0, 1)
            ...
            qry_err_list = [0.0 for q in range(len(qry_loader))]
            qry_met_list = [0.0 for q in range(len(qry_loader))]
        ...
        logging.info(
            "    Metric: %.4f +/- %.4f" % (qry_met_np.mean(), qry_met_np.std())
        )
        sys.stdout.flush()
...
def meta_test(model, head, loss_name, device, exp_group_dir, exp, sparsity_mode):
    ...
    for sup in sup_list:
        ...
        if sparsity_mode == "dense":
            ...
            exp_name = "%s_%d-shot_%s" % (
                sparsity_mode,
                sup["shots"],
                str(sup["sparsity"]).replace(" ", "_"),
            )
...
def main(
    n_shots=5,  # Number of shots of each task.
    ...
    exp_group_name = "metaoptnet_ridge_%s_%s_%s_%s" % (
        network_name,
        loss_name,
        trg_dataset,
        trg_task,
    ...
    log_path = os.path.join(exp_group_dir, "test_metaoptnet_ridge.log")
    ...
    for fold in folds:
        ...
        if "unet" in network_name:
            ...
            head = ClassificationHead(base_learner="Ridge")
        ...
        exp_pnts, exp_grid, exp_scrb, exp_cntr, exp_skel, exp_poly, exp_dens = (
            sup_qry_experiments(
                root, fold, trg_dataset, trg_task, crop_to, normalization, shots_list
            )
        )
