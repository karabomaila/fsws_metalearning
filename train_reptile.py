...
def fast_adapt(
    ...
    # Adapt the model.
    scheduler = optim.lr_scheduler.StepLR(adapt_opt, adaptation_steps // 2, gamma=0.5)
    ...
    qry_err /= qry_img.size(0) * qry_img.size(2) * qry_img.size(3)
...
def main(
    ...
    exp_group_name = "reptile_%s_%s_%s_%s" % (
        network_name,
        loss_name,
        trg_dataset,
        trg_task,
    ...
    log_path = os.path.join(exp_group_dir, "train_reptile.log")
    ...
    # Setting outer and inner optimizer.
    opt = torch.optim.SGD(model.parameters(), meta_lr)  # Original.
    ...
    while iteration <= outer_iters:
        for outer_batch in meta_loader:
            ...
            # Anneal meta-lr.
            frac_done = float(iteration) / outer_iters
            ...
            for task in range(inner_iters):
                ...
                adapt_opt = torch.optim.Adam(
                    learner.parameters(), lr=fast_lr, betas=(0, 0.999)
                )
                ...
                # Updating adaptation optimizer state.
                adapt_opt_state = adapt_opt.state_dict()
            ...
            for p in model.parameters():
                p.grad.data.mul_(1.0 / inner_iters).add_(p.data)
            opt.step()
            ...
            if iteration % test_freq == 0:
                ...
                adapt_opt_ckpt_path = os.path.join(exp_ckpt_dir, "adapt_optim.pth")

                torch.save(model, model_ckpt_path)
                torch.save(opt, optim_ckpt_path)
                torch.save(adapt_opt, adapt_opt_ckpt_path)
